...
class HasSpecUnits(object, metaclass=ABCMeta):
    """
...
class Spectrum(HasSpecUnits):
    ...
    _zqmapi = dict([(v,k) for k,v in _zqmap.items()])
    def _getZqual(self):
        return self._zqual
    def _setZqual(self,val):
        if isinstance(val,str):
            ...
            val = int(val)
            if val in list(self._zqmap.keys()):
                self._zqual = val
            else:
    ...
    def smooth(self,width=1,filtertype='gaussian',replace=True):
        ...
        smoothedflux = list(filter(self._flux,width))
        smoothederr = list(filter(err,width))
    ...
    def resample(self,newx,interpolation='linear',replace=True,**kwargs):
        ...
        if len(kwargs)>0:
            raise TypeError('unexpected keyword %s'%list(kwargs.keys())[0])
...
#        bands = bl
        ...
        scalarout = isinstance(bands,str) or isinstance(bands,phot.Band)
