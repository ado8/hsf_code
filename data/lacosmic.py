...
def lacosmic(
    ...
    AD Note: input data will fail if a ndarray with int-like dtype or np.float16 specifically.
    np.float32 works for some reason.
    ...
    for iteration in range(maxiter):
        ...
        convolved_img = ndimage.convolve(sampled_img, kernel, mode=border_mode).clip(
            min=0.0
        )
        ...
        if clean_error_image is None:
            if effective_gain is None or readnoise is None:
                raise ValueError(
                    "effective_gain and readnoise must be "
                    "input if error is not input"
                )
            med5_img = ndimage.median_filter(clean_data, size=5, mode=border_mode).clip(
                min=1.0e-5
            )
            error_image = (
                np.sqrt(effective_gain * med5_img + readnoise ** 2) / effective_gain
            )
        else:
        ...
        log.info(
            f"Iteration {iteration + 1}: Found {ncosmics} cosmic-ray "
        ...
        clean_data = _clean_masked_pixels(
            clean_data, final_crmask, size=5, exclude_mask=mask
        )
...
def _clean_masked_pixels(data, mask, size=5, exclude_mask=None):
    ...
    for coord in mask_coords:
        y, x = coord
        median_val, expanded = _local_median(data_nanmask, x, y, nx, ny, size=size)
        data[y, x] = median_val
        if expanded:
            nexpanded += 1
    if nexpanded > 0:
        log.info(f"Found {nexpanded} {size}x{size} masked regions while " "cleaning.")
    return data
...
def _local_median(data_nanmask, x, y, nx, ny, size=5, expanded=False):
    ...
    if len(goodpixels) > 0:
        ...
        median_val, expanded = _local_median(
            data_nanmask, x, y, nx, ny, size=newsize, expanded=True
        )
